...
for user in userName:
    for mention in mentions[i]:
        if mention in userName:
            count += 1
            G.add_edge(mention, user)
    i += 1
print(count)
...
nx.draw(G, pos, node_color='green', node_size=30, edge_color='grey')
plt.show()
...
def cliques(G):
    ...
    a = nx.find_cliques(G)
    i = 0
    # print the members and the total number of communities
    for clique in a:
        # print(clique)
        i += 1
    total_comm_max_cl = i
    ...
    # Remove "len(clique)>1"
    cliques = [c for c in nx.find_cliques(G) if len(clique) > 1]

    # plotting
    pos = nx.spring_layout(G)
    plt.clf()
    nx.draw(G, pos=pos, with_labels=False,
            node_size=10, edge_coloe='grey')
    nx.draw(G, pos=pos, nodelist=cliques[0],
            node_color='b', node_size=10, edge_color='grey')
    nx.draw(G, pos=pos, nodelist=cliques[1],
            node_color='y', node_size=10, edge_color='grey')
    plt.suptitle('Total number of communities: ' +
                 str(total_comm_max_cl), fontsize=10, fontname='Arial')
    plt.show()
